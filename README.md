...
標準的な閉域インストール手順書です。既存の DNS / NTP の利用に加えて、必要なパッケージの持ち込みと導入も前提に整理しており、外部側での資材取得、閉域搬入、ミラーレジストリー準備、`install-config.yaml` と `agent-config.yaml` の作成、Agent ISO 生成、インストール確認までを扱います。
