...
| -------------- | -------------------------------------- | --------- | ----------------------------------------------------- |
| 外部インターネット接続ホスト | Red Hat配布サイト、公開レジストリー、利用するRHELパッケージ提供元 | TCP 443   | OpenShift CLI、OCPリリース、Operator、追加イメージ、必要なRHELパッケージの取得 |
...
- `pkg`：ダウンロードしたアーカイブと、必要に応じて搬送するRPMを置く。
...
> [!TIP]
> **オプション：閉域内作業ホスト用RPMを準備する**
> 閉域内作業ホストが必要なRHELパッケージを閉域内のリポジトリーやSatelliteなどから導入できない場合は、外部インターネット接続ホストで必要なRPMとローカルRPMリポジトリーを作成し、閉域内へ搬送する。

> [!NOTE]
> **注釈**
> 外部インターネット接続ホストと閉域内作業ホストは、同じRHEL major／minor、アーキテクチャー、リポジトリー世代を使用する。

次のコマンドは、閉域内作業ホストでのCLI操作、レジストリー接続、DNS／NTP同居構成に必要なパッケージを取得する。`tmux`の利用は任意である。OpenSSHは版競合を避けるため対象から除外する。

**実行場所：外部インターネット接続ホスト**

```bash
# RPMの取得とリポジトリー作成に使用するコマンドを導入する
sudo dnf install -y dnf-plugins-core createrepo_c

# 搬送用RPMの配置先を作成する
RPM_REPO="$PKG_DIR/rpms-required"
mkdir -p "$RPM_REPO"

# 閉域内作業ホストで使用するRPMと依存RPMを取得する
sudo dnf download \
  --resolve \
  --alldeps \
  --exclude='openssh*' \
  --destdir "$RPM_REPO" \
  bind bind-utils chrony curl firewalld jq nmstate \
  openssl podman tar tmux

# DNFから参照できるリポジトリーメタデータとチェックサムを作成する
sudo chown -R "$(id -u):$(id -g)" "$RPM_REPO"
createrepo_c "$RPM_REPO"
(cd "$RPM_REPO" && sha256sum ./*.rpm > SHA256SUMS)
```

作成したリポジトリーにRPMとmetadataがあることを確認する。

```bash
find "$RPM_REPO" -maxdepth 1 -type f -name '*.rpm' | head
ls -lh "$RPM_REPO/repodata/repomd.xml" "$RPM_REPO/SHA256SUMS"
```
...
- `pkg/`の各アーカイブと、`rpms-required/`（※作成した場合）
...
> [!TIP]
> **オプション：承認済みSSH経路で直接搬送する**
> テスト環境などで、外部インターネット接続ホストから閉域内作業ホストへのSSH転送が可能な場合は、`scp`で資材を直接搬送できる。`scp`は転送中のファイル名と進捗を表示する。

ユーザー名、接続先、配置先は使用する値へ変更する。

**実行場所：外部インターネット接続ホスト**

```bash
TRANSFER_TARGET="<SSHユーザー>@<閉域内作業ホストのFQDNまたはIP>"
TRANSFER_DEST="<閉域内作業ホスト上の配置先>/ocp-airgap"

ssh "$TRANSFER_TARGET" "mkdir -p '$TRANSFER_DEST'"
scp -rp "$AIRGAP_ROOT/." \
  "${TRANSFER_TARGET}:${TRANSFER_DEST%/}/"
```
...
第3章のオプションでRPMを作成した場合は、搬入後のファイルが取得時と一致することも確認する。

```bash
(cd "$PKG_DIR/rpms-required" && sha256sum -c SHA256SUMS)
```
...
本手順書では、DNS、NTP、ミラーレジストリーを閉域内作業ホストへ同居させる。
...
まず、閉域内作業ホストで使用する汎用パッケージを準備し、搬入済みの`oc`、`kubectl`、`openshift-install`、`oc-mirror`を配置する。

閉域内作業ホストには、実施する作業に応じて次のパッケージまたは同等機能を導入する。

`bind-utils`、`chrony`、`curl`、`firewalld`、`jq`、`nmstate`、`openssl`、`podman`、`tar`

> [!TIP]
> **オプション**
> 閉域内作業ホストにDNSを同居させる場合は`bind`、長時間処理の画面を保持する場合は`tmux`を用意する。

> [!TIP]
> **オプション：ローカルRPMリポジトリーから不足パッケージを導入する**
> 第3章のオプションでローカルRPMリポジトリーを作成して搬入した場合は、搬入済みディレクトリーをDNFへ登録し、閉域内作業ホストに不足しているパッケージだけを導入する。既に導入済みのパッケージは更新対象にしない。

**実行場所：閉域内作業ホスト**

```bash
# 搬入したローカルRPMリポジトリーをDNFへ登録する
LOCAL_REPO_DIR="$PKG_DIR/rpms-required"
test -s "$LOCAL_REPO_DIR/repodata/repomd.xml"

sudo tee /etc/yum.repos.d/ocp-airgap-local.repo >/dev/null <<EOF_REPO
[ocp-airgap-local]
name=OCP air-gap local RPM repository
...
# 不足しているパッケージをローカルリポジトリーから導入する
if ((${#MISSING[@]})); then
  sudo dnf \
    --disablerepo='*' \
    --enablerepo='ocp-airgap-local' \
    --exclude='openssh*' \
    install -y "${MISSING[@]}"
fi
```
...
Leap status     : Normal
running
<servicesの一覧。少なくとも ssh dns ntp を含む>
<portsの一覧。少なくとも 8443/tcp を含む>
```

同じ名前解決と時刻同期を、Agent ISOから起動する全OpenShiftノードからも実施できる通信設計にする。

> [!TIP]
> **オプション：DNSとNTPを閉域内作業ホストへ同居させる場合の設定**

**SNO向けの同居構成を一括設定する**

本項では、閉域内作業ホストへSNO用DNS、NTPを同居させ、firewalldを有効化する。`UPSTREAM_NTP`だけを組織内の上位NTPへ変更して、一括実行する。

**実行場所：DNS、NTP、ミラーレジストリーを同居させる閉域内作業ホスト**

```bash
UPSTREAM_NTP="<組織内上位NTPのIPまたはFQDN>"

sudo bash -s -- "$UPSTREAM_NTP" <<'SCRIPT'
set -euo pipefail

UPSTREAM_NTP="$1"

# 本手順書のサンプル値を設定する
WORK_IP="192.168.100.10"
MACHINE_CIDR="192.168.100.0/24"
DNS_ZONE="ocp.lab.example.com"
REVERSE_ZONE="100.168.192.in-addr.arpa"
MIRROR_PORT="8443"

# BINDの待受けアドレスと問い合わせ許可範囲を設定する
sed -Ei \
  's|^[[:space:]]*listen-on port 53[[:space:]]*\{[^}]*\};|        listen-on port 53 { 127.0.0.1; 192.168.100.10; };|' \
  /etc/named.conf
sed -Ei \
  's|^[[:space:]]*allow-query[[:space:]]*\{[^}]*\};|        allow-query { localhost; 192.168.100.0/24; };|' \
  /etc/named.conf

# 正引き・逆引きゾーンをBINDへ登録する
cat > /etc/named/ocp-airgap.zones <<'EOF_ZONES'
zone "ocp.lab.example.com" IN {
  type master;
  file "ocp.lab.example.com.zone";
};
...
grep -qF 'include "/etc/named/ocp-airgap.zones";' /etc/named.conf || \
  echo 'include "/etc/named/ocp-airgap.zones";' >> /etc/named.conf

# SNOで使用する正引きゾーンを作成する
cat > /var/named/ocp.lab.example.com.zone <<'EOF_FORWARD'
$TTL 300
@ IN SOA dns.ocp.lab.example.com. hostmaster.ocp.lab.example.com. (
  2026082001 3600 900 604800 300 )
  IN NS dns.ocp.lab.example.com.

work      IN A 192.168.100.10
dns       IN A 192.168.100.10
registry  IN A 192.168.100.10
sno-0     IN A 192.168.100.20
api       IN A 192.168.100.20
api-int   IN A 192.168.100.20
*.apps    IN A 192.168.100.20
EOF_FORWARD

# SNOで使用する逆引きゾーンを作成する
cat > /var/named/192.168.100.rev <<'EOF_REVERSE'
$TTL 300
@ IN SOA dns.ocp.lab.example.com. hostmaster.ocp.lab.example.com. (
  2026082001 3600 900 604800 300 )
  IN NS dns.ocp.lab.example.com.

10 IN PTR work.ocp.lab.example.com.
10 IN PTR dns.ocp.lab.example.com.
10 IN PTR registry.ocp.lab.example.com.
20 IN PTR sno-0.ocp.lab.example.com.
20 IN PTR api.ocp.lab.example.com.
20 IN PTR api-int.ocp.lab.example.com.
EOF_REVERSE

# ゾーンファイルの権限と構文を確認する
chown root:named \
  /var/named/ocp.lab.example.com.zone \
...
named-checkconf
named-checkzone "$DNS_ZONE" /var/named/ocp.lab.example.com.zone
named-checkzone "$REVERSE_ZONE" /var/named/192.168.100.rev

# 閉域内作業ホストをNTPサーバーとして設定する
grep -qF "server $UPSTREAM_NTP iburst" /etc/chrony.conf || \
  echo "server $UPSTREAM_NTP iburst" >> /etc/chrony.conf
grep -qF "allow $MACHINE_CIDR" /etc/chrony.conf || \
  echo "allow $MACHINE_CIDR" >> /etc/chrony.conf

# 使用中のNetworkManager接続とfirewalld zoneを確認する
DEV="$(ip route show default | awk '{print $5; exit}')"
CON="$(nmcli -g GENERAL.CONNECTION device show "$DEV")"

# SSH、DNS、NTP、ミラーレジストリーの通信を許可する
systemctl enable --now firewalld
ACTIVE_ZONE="$(nmcli -g connection.zone connection show "$CON")"
if [ -z "$ACTIVE_ZONE" ] || [ "$ACTIVE_ZONE" = "--" ]; then
  ACTIVE_ZONE="$(firewall-cmd --get-default-zone)"
fi

firewall-cmd --zone="$ACTIVE_ZONE" --permanent --add-service=ssh
firewall-cmd --zone="$ACTIVE_ZONE" --permanent --add-service=dns
firewall-cmd --zone="$ACTIVE_ZONE" --permanent --add-service=ntp
firewall-cmd --zone="$ACTIVE_ZONE" --permanent --add-port="${MIRROR_PORT}/tcp"
firewall-cmd --reload

# BINDとchronydを起動する
systemctl enable named chronyd
systemctl restart named chronyd

# 閉域内作業ホスト自身も作成したDNSを参照する
...
設定後、SNO用の名前解決と時刻同期を確認する。

```bash
cat /etc/resolv.conf
dig +short registry.ocp.lab.example.com
dig +short @192.168.100.10 api.ocp.lab.example.com
dig +short @192.168.100.10 api-int.ocp.lab.example.com
dig +short @192.168.100.10 test.apps.ocp.lab.example.com
dig +short @192.168.100.10 -x 192.168.100.20
chronyc tracking
```
...
> [!TIP]
> **オプション：SNO用install-config.yamlを一括作成する**
> 本手順書のサンプル値を使用してISO作成まで確認する場合は、CA、IDMS、Pull secret、SSH公開鍵を各ファイルから読み込み、SNO用`install-config.yaml`を一括作成できる。実行場所は閉域内作業ホストである。

```bash
LOG="$HOME/ocp-airgap/install-config-build.log"
...
(
  ...
  SSH_PRIVATE_KEY="$HOME/.ssh/cluster-node-debug"
  SSH_PUBLIC_KEY="${SSH_PRIVATE_KEY}.pub"
  IDMS_FILES=("$MIRROR_WORK_DIR/working-dir/cluster-resources"/idms-*.yaml)
  INSTALL_CONFIG="$AIRGAP_ROOT/install-config.yaml"

  # install-config.yamlの作成に必要なファイルを確認する
  test -s "$MIRROR_REGISTRY_CA" || {
    echo "ERROR: CA証明書がありません: $MIRROR_REGISTRY_CA" >&2
    exit 1
  }
  test -s "$CLUSTER_PULL_SECRET" || {
    echo "ERROR: Pull secretがありません: $CLUSTER_PULL_SECRET" >&2
    exit 1
  }
  test -e "${IDMS_FILES[0]}" || {
    echo "ERROR: IDMSがありません: $MIRROR_WORK_DIR/working-dir/cluster-resources/idms-*.yaml" >&2
    exit 1
  }

  # 閉域内作業ホストからRHCOSノードへ接続するSSH鍵を準備する
  mkdir -p "$HOME/.ssh"
  chmod 700 "$HOME/.ssh"

  if [ ! -s "$SSH_PRIVATE_KEY" ]; then
    if [ -e "$SSH_PUBLIC_KEY" ]; then
      echo "ERROR: 公開鍵だけが存在します: $SSH_PUBLIC_KEY" >&2
      exit 1
    fi
    ssh-keygen -t ed25519 -N '' -f "$SSH_PRIVATE_KEY" -C 'cluster-node-debug'
  fi
  ...
  # IDMSから指定したsourceに対応するmirrorを取得する
  extract_first_mirror() {
    local wanted="$1"
    awk -v wanted="$wanted" '
      function reset_item() {
        source = ""
      ...
      function emit_item() {
        if (source == wanted && mirror != "") {
          print mirror
          found = 1
          exit
        }
      }
      /^[[:space:]]*-[[:space:]]+(source|mirrors):/ {
        if (started) emit_item()
        reset_item()
        started = 1
      }
      {
        line = $0
        if (started && line ~ /^[[:space:]]*(-[[:space:]]*)?source:[[:space:]]*/) {
          sub(/^[[:space:]]*(-[[:space:]]*)?source:[[:space:]]*/, "", line)
          source = line
        }
        if (started && line ~ /^[[:space:]]*(-[[:space:]]*)?mirrors:[[:space:]]*$/) {
          in_mirrors = 1
          next
        }
        if (started && in_mirrors && line ~ /^[[:space:]]*-[[:space:]]*/) {
          sub(/^[[:space:]]*-[[:space:]]*/, "", line)
          if (line !~ /^(source|mirrors):/) {
  ...
  MIRROR_ART="$(extract_first_mirror \
    'quay.io/openshift-release-dev/ocp-v4.0-art-dev')"
  MIRROR_RELEASE="$(extract_first_mirror \
    'quay.io/openshift-release-dev/ocp-release')"

  test -n "$MIRROR_ART" || {
    echo "ERROR: ocp-v4.0-art-devのmirrorをIDMSから取得できませんでした" >&2
    exit 1
  }
  test -n "$MIRROR_RELEASE" || {
    echo "ERROR: ocp-releaseのmirrorをIDMSから取得できませんでした" >&2
    exit 1
  }

  # Pull secretとSSH公開鍵を読み込む
  PULL_SECRET="$(jq -c . "$CLUSTER_PULL_SECRET")"
  SSH_KEY="$(cat "$SSH_PUBLIC_KEY")"

  # SNO用install-config.yamlを作成する
  cat > "$INSTALL_CONFIG" <<EOF_INSTALL
apiVersion: v1
...
EOF_INSTALL
  ...
  printf 'release mirror 1: %s\n' "$MIRROR_ART"
  printf 'release mirror 2: %s\n' "$MIRROR_RELEASE"

  grep -E '^(baseDomain:|  name:|imageDigestSources:|  - source:)' \
    "$INSTALL_CONFIG"
...
if [ "$RC" -eq 0 ]; then
  echo 'install-config.yamlの作成に成功しました'
else
  echo 'install-config.yamlの作成に失敗しました'
  tail -n 30 "$LOG"
fi
```
...
> [!TIP]
> **オプション：ISO作成の構文確認用SNOファイルを作成する**
> 実機を起動せずにAgent ISOの作成処理まで確認する場合は、有効形式のサンプルNIC、MACアドレス、ディスク名を使用した`agent-config.yaml`を一括作成できる。このファイルの値は実機と一致しないため、作成したISOを実際のOpenShiftノードの起動には使用できない。

**実行場所：閉域内作業ホスト**

```bash
cat > "$HOME/ocp-airgap/agent-config.yaml" <<'EOF_AGENT'
apiVersion: v1beta1
kind: AgentConfig
metadata:
  name: ocp
